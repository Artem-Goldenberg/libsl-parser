...
type HashMapInOuParams <K, V> is java.util.HashMap for java.util.Map  where K: in any, K: out any, V: out any{
...
define action <T, R> COMPLICATED_RETURN_TYPE_OF_GENERIC_ACTION(
    x: Int,
    s: T
): HashMap<HashMap<T, R>, HashMap<T, R>> where T: any, R: string;
automaton A (var storage: array<K>) : HashMap<K, V> {
